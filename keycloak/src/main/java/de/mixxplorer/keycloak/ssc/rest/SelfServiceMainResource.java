...
public class SelfServiceMainResource {
    ...
    public Object getSelfServiceResources() {
        ...
        final HttpResponse response = keycloakSession.getContext().getHttpResponse();
        ...
        // Do not continue, if the SSC_CLIENT_ID client is not available as this is an
        ...
        // Handle all CORS OPTIONS requests. As we are having the same CORS options for
        // all routes, we return always a valid CORS response, even if the route does
        // not exist. This might be a bit misleading, but keeps the code tidy.
        // Furthermore, Keycloak is internally doing the same with its Admin API.
        // see also org.keycloak.services.resources.admin
        if (request.getHttpMethod().equalsIgnoreCase(HttpMethod.OPTIONS)) {
            return new SelfServicePreflight().answerCors();
        }

        // Check whether the user is authenticated. Doing this here ensures all requests
        // forwarded to our real resources are authenticated.
        ...
        // Add cors options to prepared response, which will be used (implicitly
        // apparently) when building responses downstream.
        cors.allowedOrigins(authResult.getToken()).add(Response.ok(response));
    ...
    // Users must be in the SSC_MANAGER_CLIENT_ROLE client role of client
    // SSC_CLIENT_ID
    private AuthenticationManager.AuthResult checkPermissionsAndGetUser(ClientModel sscClient) {
        ...
        if (authResult == null) {
            throw new NotAuthorizedException("Not authorized");
                ...
                || !authResult.getToken().getResourceAccess(sscClient.getClientId())
                        .isUserInRole(SSC_MANAGER_CLIENT_ROLE)) {
